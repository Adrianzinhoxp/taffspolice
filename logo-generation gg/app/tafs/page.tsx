  ...
  useEffect(() => {
    const loadTafs = async () => {
      try {
        ...
        const approvedTafs = data.tafs.filter((taf: TAF) => taf.status.approved)
        setTafs(approvedTafs)
      } catch (error) {
    ...
    <div className="min-h-screen page-overlay">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-[#e2e8f0] mb-2">TAFs Realizados</h1>
            <p className="text-[#5a6478]">Historico de testes de aptidao fisica aprovados</p>
          </div>
          <Link href="/">
        ...
        {tafs.length === 0 ? (
          <Card className="border-[#2a2e3a] card-modern">
            <CardContent className="flex flex-col items-center justify-center py-12">
              <p className="text-[#5a6478] text-lg">Nenhum TAF aprovado ainda</p>
              <Link href="/">
                <Button className="mt-4 bg-[#2a3040] hover:bg-[#343a4c] text-[#e2e8f0] border border-[#3a4050]">Ir para Recrutamento</Button>
              </Link>
            </CardContent>
          </Card>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {currentTafs.map((taf) => (
                <Card
                  key={taf.id}
                  className="border-[#2a2e3a] hover:border-[#3a4050] transition-all cursor-pointer card-modern"
                ...
                >
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      ...
                      {taf.photo && (
                        <div className="w-16 h-16 rounded-xl overflow-hidden border-2 border-[#2a2e3a]">
                          <img
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <Button
                      ...
                      className="w-full bg-[#2a3040] hover:bg-[#343a4c] text-[#c8d0dc] border border-[#3a4050]"
                      size="sm"
                    >
      ...
      <Dialog open={!!selectedTaf} onOpenChange={() => setSelectedTaf(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-[#161a22] border-[#2a2e3a]">
          <DialogHeader>
            <DialogTitle className="text-2xl text-[#e2e8f0]">Detalhes do TAF - Aprovado</DialogTitle>
          </DialogHeader>
          ...
          {selectedTaf && (
            <div className="space-y-6">
              <div className="flex gap-6 items-start">
                {selectedTaf.photo && (
                  <div className="w-48 h-48 rounded-xl overflow-hidden border-2 border-[#2a2e3a] flex-shrink-0">
                    <img
                  ...
                  <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/15">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-lg text-emerald-400">APROVADO</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <Button
                ...
                className="w-full bg-[#2a3040] hover:bg-[#343a4c] text-[#e2e8f0] border border-[#3a4050]"
              >
